...
class TransMilPlipFinetune(nn.Module):
    def __init__(self, vocab_size, max_seq_len, f_dim, d_model, vocab=None):
        super(TransMilPlipFinetune, self).__init__()
        ...
        self.load_state_dict(ckpt_weights)

        # Freeze head classifier
        self._fc2.weight.requires_grad_(False)
        self._fc2.bias.requires_grad_(False)
...
if __name__ == "__main__":
    ...
    model = TransMilPlipFinetune(n_classes=2).cuda()
    print(model.eval())
